...
function sendAttributeBufferToGLSL(data, dataCount, attribName) {
	...
	var aN = gl.getAttribLocation(gl.program, attribName);
	if (aN < 0) {
  ...
  // Create a buffer object
	...
	// Assign the buffer object to attribName variable
	gl.vertexAttribPointer(aN, dataCount, gl.FLOAT, false, 0, 0);

	// Enable the assignment to attribName variable
...
function sendNormalBufferToGLSL(vertexData, dataCount, attribName)
{
    ...
    for (var i = 0; i < vertexData.length; i++)
    {
        data.push(vertexData[i].normal.elements[0]);
		data.push(vertexData[i].normal.elements[1]);
		data.push(vertexData[i].normal.elements[2]);      
    }
